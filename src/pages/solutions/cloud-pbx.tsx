...
import { CLOUD_PBX_QUERY } from "@/sanity/queries";

const CloudPBXPage = () => {
  const [data, setData] = useState<any>(null);

  useEffect(() => {
    sanityFetch({ query: CLOUD_PBX_QUERY })
      .then((res) => {
        console.log("CLOUD PBX DATA:", res);
        setData(res);
      })
      ...
      <section className="py-16 sm:py-20">
        <div className="container-custom">
          ...
          <div className="mt-12 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {data?.keyFeaturesSection?.features?.map((feature: any) => (
          ...
          <div className="mt-8">
            <Button asChild size="lg">
              <Link to={data?.ctaSection?.buttonLink || "/contact"}>
                {data?.ctaSection?.buttonText || "Request a Demo"}
              </Link>
            </Button>
          </div>
        </div>
